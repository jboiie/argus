...
- **Security (public repo)**: the repo is public and Supabase's anon key ships client-side by design — without Row Level Security this is a full read/write hole (see [CVE-2025-48757](https://nvd.nist.gov/vuln/detail/CVE-2025-48757), 303 exposed endpoints across 170 apps from exactly this misconfiguration). Before the repo goes public: never commit keys (`.env` stays gitignored, `web/.env` too); enable RLS on every table, no exceptions; give the dashboard's anon key a `SELECT`-only policy; keep `INSERT`/`UPDATE` behind the service-role key, used only by harness/sentinel backend scripts, never in `web/`'s code path.
- **No unsafe HTML rendering**: this is a code-level rule, not a per-field data-model one, because every attacker-influenced free-text field is a candidate — `Product.description`, `Attack Event.prompt`/`response`, `Session Turn.content`. A prompt-injection payload is often literally designed to look like markup or instructions. `web/` never uses `dangerouslySetInnerHTML` when rendering any of these — see its README's "Rendering untrusted text".
...
**Graceful-degradation behavior rule** (reference agent, not the drift sentinel itself): before answering a question, or before authorizing a new Mandate, that touches a `ground_truth_ref` with an unresolved critical incident (`severity = critical` and `reviewed_at IS NULL`), the agent must decline to confirm the claim or hold the Mandate — never repeat the possibly-wrong value. This is the project's one concrete "failure recovery" behavior, demonstrated live rather than just logged. **Implemented in `agent/drift_guard.py`** — `build_system_prompt` declines to confirm a flagged product/policy, `execute_tool_call` hard-blocks Mandate creation for a cart item touching one (see BUGS.md, 2026-08-26). Nothing ever calls `mark_drift_incident_reviewed` automatically — `scripts/mark_reviewed.py` is the manual review path, run before a demo if test noise has accumulated.
