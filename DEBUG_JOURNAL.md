...
## 2026-08-24 — Custom commerce vulnerabilities (step 12): two more Groq rate-limit layers found

**What broke:** wiring the 4 commerce-specific `CustomVulnerability` objects (Price Integrity, Catalog Injection, Refund Abuse, Mandate Bypass). First run: 6/7 test cases errored completely, even though the exact same code called in isolation (`price_integrity.a_simulate_attacks()` alone) succeeded every time.

**Why (1):** `max_concurrent` on `red_team()` bounds DeepTeam's own task parallelism, not real Groq API concurrency. Several vulnerabilities' attack-generation + judge calls ran "concurrently" per DeepTeam but all shared one Groq key's 8000 TPM budget — each hit 429, each retried independently, and the retries piled back onto the same recovering budget faster than it could refill. A thundering herd, invisible when testing one vulnerability alone.

**Fix (1):** Added a process-wide `asyncio.Semaphore(1)` inside `GroqModel.a_generate`, serializing actual outbound Groq calls no matter how many DeepTeam tasks are notionally running at once. Rerun: 5/7 succeeded (up from 1/7).

**Why (2):** The remaining 2 errors (`Refund Abuse`, both types) turned out to be a completely different, unrelated cause: Groq's free tier also has a **200,000 tokens/day** cap (TPD) on `gpt-oss-20b`, separate from the 8000/minute cap. A full day of build/debug testing (every `run_asi`/`run_custom`/isolated-debug-script call) had used 198,802 of it — `Refund Abuse` simply ran last and hit an almost-empty daily budget. Confirmed by calling it in isolation moments later: worked fine, same code.

**Fix (2):** None available on the free tier — this is a hard quota, not a bug. Stopped further live Groq calls for the day rather than pay a ~24-minute wait per retry for a nearly-exhausted rolling window.

**Real consequence for step 15:** if a day of *build-time* testing came within 1% of the daily free-tier cap, the actual "hundreds of attempts across categories" full-volume run (Section 4.3) will not fit inside the Groq free tier at all. This is now a concrete, quantified reason to budget for Groq's paid Developer tier before step 15, not just a hedge. Full detail in `BUGS.md`.
